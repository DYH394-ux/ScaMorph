...
parser.add_argument('--atlas_dir', default=r'D:\data\IXI_data\IXI_data (2)\IXI_data\atlas.pkl', help='atlas_dir files')
parser.add_argument('--train_dir', default=r'D:\data\IXI_data\IXI_data (2)\IXI_data\Train',help='Train_dir path')
parser.add_argument('--val_dir', default=r'D:\data\IXI_data\IXI_data (2)\IXI_data\Val', help='Val_dir path')
parser.add_argument('--model_name', default='ScaMorph',help='model output directory (default: models)')
parser.add_argument('--n_classes', type=int, default=32,required=True, help='number of output classes for the model')
# training parameters
...
def main():
    ...
    Initialize model
    '''
    # if hasattr(args, 'n_classes'):
    #     model = ScaMorph(args.img_size, args.n_classes)  # 在这里传递 n_classes
    # else:
    #     # 如果 args 中没有 n_classes，则进行回退或报错
    #     # 例如，如果对于这个特定的训练脚本它是一个固定数字：
    #     # num_output_classes = 2 # 或者你的特定任务需要的任何数字
    #     # model = ScaMorph(args.img_size, num_output_classes)
    #     # 或者抛出一个错误：
    #     raise ValueError("args.n_classes 未定义。请确保它已作为脚本参数传入。")
    model = ScaMorph(args.img_size,args.n_classes)
    model.cuda()
    ...
    for epoch in range(epoch_start, args.epochs):
        ...
        for data in train_loader:
            ...
            loss = loss_sim * weights[0] +  loss_reg* weights[1]
            ...
            sys.stdout.write("\r" + 'Epoch {}/{} idx {}: Total_loss: {:.4f} Sim: {:.6f} Reg: {:.6f} {:.4f} sec/step '.format(epoch,args.epochs,idx, epoch_total_loss.val, sim_loss.val, reg_loss.val,epoch_step_time.val))
            sys.stdout.flush()            
            del output
