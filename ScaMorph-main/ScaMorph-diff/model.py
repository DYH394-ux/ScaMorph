...
class ScaMorph_diff(nn.Module):
    def __init__(self, inshape):
        super(ScaMorph_diff, self).__init__()
        bias_opt = True
        ...
        self.flow_mean = Conv(16, ndims, kernel_size=3, padding=1)
        self.flow_log_sigma = Conv(16, ndims, kernel_size=3, padding=1)
        ...
        self.diffeomorph_transform = DiffeomorphicTransform(size=self.inshape, mode='bilinear',time_step=7)
        self.spatial_transform = SpatialTransformer(self.inshape)
    ...
    def forward(self, x):
        ...
        # flows
        flows = self.diffeomorph_transform(z)

        # image warping
        y_src = self.spatial_transform(source, flows)

        flow_param = torch.cat((flow_mean, flow_log_sigma), dim=1)
            

        return y_src, flow_param, flows
